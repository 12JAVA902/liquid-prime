        ...
        >
          <div className="flex items-center justify-between px-5 py-4 relative z-10">
            <div className="flex items-center gap-2">
              ...
              {speaking && <Volume2 className="w-4 h-4 text-primary animate-pulse" />}
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-2 space-y-3 relative z-10">
            {messages.length === 0 && (
              <div className="text-center text-muted-foreground text-sm mt-8">
                ...
                <p className="text-xs mt-1">🎤 Use voice input or type</p>
              </div>
            )}
            {messages.map((msg, i) => (
            ...
            {loading && messages[messages.length - 1]?.role === "user" && (
              <div className="flex justify-start">
                <div className="liquid-glass-subtle rounded-2xl px-4 py-2.5 text-sm text-muted-foreground relative z-10">Thinking...</div>
              </div>
            )}
            <div ref={bottomRef} />
          </div>
          ...
          <div className="px-4 py-3 relative z-10">
            <div className="flex gap-2">
              <button
                ...
                className={`depth-press w-11 h-11 rounded-2xl flex items-center justify-center ${listening ? "bg-destructive" : "liquid-glass-subtle"}`}
              >
                {listening ? <MicOff className="w-4 h-4 text-destructive-foreground" /> : <Mic className="w-4 h-4 text-foreground" />}
              </button>
              <input
                value={input}
                onChange={e => setInput(e.target.value)}
                onKeyDown={e => e.key === "Enter" && send()}
                placeholder={listening ? "Listening..." : "Message Prime AI..."}
